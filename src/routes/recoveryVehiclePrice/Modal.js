...
const modal = ({
  ...
  let from = recoveryVehiclePriceList.map(item => (
    <FormItem  key={item.id} label={item.priceName} hasFeedback {...formItemLayout} style={{"marginBottom": "8px"}}>
      ...
      })(<InputNumber min={0} max={100000} precision={2}/> )}{item.id === 4 ?'元/台':'元/公里'}
    </FormItem>))
  ...
  const handleOk = () => {
    ...
    validateFields((errors) => {
      ...
      const d = recoveryVehiclePriceList.map(item => {
...
modal.propTypes = {
  ...
  onOk: PropTypes.func,
  recoveryVehiclePriceList:PropTypes.array,
}
